...
def generate_report():
    ...
    try:
        print("📝 Recebida solicitação de relatório")
        try:
        ...
        for item in ranking:
            ...
            name_display = f"{name} ({coord})" if coord else name
            
            table_data.append([
                str(item.get('position', '-')),
                str(item.get('categoria', item.get('cluster', 'N/A')) or 'N/A'),
                name_display,
                str(item.get('nota', '-')),
                str(item.get('margemErro', '-'))
            ])
        ...
        if graph_image_base64:
            try:
                ...
                if ',' in graph_image_base64:
                    graph_image_clean = graph_image_base64.split(',', 1)[1]
                elif 'base64,' in graph_image_base64:
                    graph_image_clean = graph_image_base64.split('base64,', 1)[1]
        ...
        def email_thread():
            try:
                ...
                print(f"✅ Email enviado com sucesso (background)!")
            except Exception as e:
                print(f"❌ Erro ao enviar email (background): {e}")
                # Log error to file
                try:
                    with open('backend_error.log', 'a') as f:
                        f.write(f"\n\n--- Email Error at {datetime.now().isoformat()} ---\n")
                        f.write(str(e) + "\n")
                except:
                    pass

        threading.Thread(target=email_thread).start()
        print(f"📧 Disparado envio de email em background")
