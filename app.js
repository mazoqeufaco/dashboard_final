...
function renderTree(itemMap, decimals){
      ...
      if(branch.solutions && branch.solutions.length > 0){
        result.children = branch.solutions.map((coordStr, solIndex) => {
          ...
          if(!item){
            const variations = [
              coordStr + '.a',
              coordStr.replace(/(\d+)([a-z])/i, '$1.$2'),
              coordStr.replace(/\.(\d+)$/, '.$1.a'),
            ];
  ...
  // Função para obter cor baseada na coordenada (baseado na matriz)
  function getColorForCoord(coordStr) {
    if(!coordStr) return null;
    // I.1, I.2, I.3: vermelho
    if(/^I\.(1|2|3)$/.test(coordStr)) return '#ff4444';
    // II.2, II.2.a, II.3, II.2.b: azul (IA por API com anonimização)
    // Mapeamento: II.2.a -> II.2, II.2.b -> II.3
    if(/^II\.(2|2\.a|3|2\.b)$/.test(coordStr) || coordStr === 'II.2' || coordStr === 'II.3') return '#4299e1';
    // III.1a: azul (IA própria sem engenharia reversa)
    if(/^III\.1a$/.test(coordStr) || coordStr === 'III.1a') return '#4299e1';
    // III.1.b: laranja (PLIM 2.0 DMZ na SEG)
    if(/^III\.1\.b$/.test(coordStr) || coordStr === 'III.1.b') return '#ff8c00';
    // III.2.a, III.2.b: laranja (IA própria com engenharia reversa)
    if(/^III\.2\.(a|b)$/.test(coordStr)) return '#ff8c00';
    // III.2.c: azul (PLIM 2.0 externo)
    if(/^III\.2\.c$/.test(coordStr)) return '#4299e1';
  ...
  function update(source){
    ...
    // Encurtar ramos terminais: usar espaçamento menor para folhas
    nodes.forEach(d => { 
      if(d.isLeaf) {
        // Folhas: espaçamento reduzido
        d.y = (d.depth - 1) * 180 + 120;
      } else {
        // Nodos internos: espaçamento normal
        d.y = d.depth * 180;
      }
    });
    ...
    nodeEnter.append('circle')
      ...
      .style('fill', d => {
        if(d.isLeaf && d.coordStr) {
          const color = getColorForCoord(d.coordStr);
          return color || '#fff';
        }
        return d._children ? '#cfe2ff' : '#fff';
      })
      .style('stroke', '#333')
    ...
    const textEnter = nodeEnter.append('text')
      ...
      .style('fill', d => {
        if(d.isLeaf && d.coordStr) {
          const color = getColorForCoord(d.coordStr);
          return color || (d.isLeaf ? '#60a5fa' : '#888');
        }
        return d.isLeaf ? '#60a5fa' : '#888';
      })
      .text(d => {
        if(d.isLeaf && d.coordStr){
          return `${d.name} (${d.coordStr})`;
        }
        return d.name;
      })
      .on('click', function(d){
    ...
    // Tags removidos dos nodos - agora só aparecem nos links
    ...
    // Adicionar labels nos links (sobre os ramos, longe dos nodos)
    ...
    linkLabelsEnter.transition().duration(duration)
      .style('opacity', 1);

    linkLabels.transition().duration(duration)
      .attr('transform', d => {
        // No D3 tree: x é vertical, y é horizontal
        // Posiciona no meio do link, mas com offset para ficar mais perto do nodo de origem
        // Isso mantém o label longe do nodo de destino
        const sourceX = d.source.x; // vertical
        const sourceY = d.source.y; // horizontal
        const targetX = d.target.x; // vertical
        const targetY = d.target.y; // horizontal
        
        // Calcula o ponto médio
        const midX = (sourceX + targetX) / 2;
        const midY = (sourceY + targetY) / 2;
        
        // Offset: move 35% na direção do nodo de origem para ficar longe do destino
        const offsetX = (sourceX - targetX) * 0.35;
        const offsetY = (sourceY - targetY) * 0.35;
        
        const labelX = midX + offsetX;
        const labelY = midY + offsetY;
        
        // Não rotaciona - mantém horizontal para melhor legibilidade
        return `translate(${labelY},${labelX})`;
      })
      .attr('dy', -12) // Offset vertical para ficar acima do link
      .text(d => {
        const target = d.target;
        if(target.tag) return target.tag;
        return '';
    ...
    linkLabels.exit().transition().duration(duration)
      .style('opacity', 0)
      .remove();
  ...
  const legendItems = [
    { coord: 'I.1, I.2, I.3', color: '#ff4444', label: 'confia? essas empresas terão dados da SEG' },
    { coord: 'II.2, II.3, III.1a, III.2.c', color: '#4299e1', label: 'plataformas com login a partir de qualquer lugar, e em qualquer horário' },
    { coord: 'III.1.b, III.2.a, III.2.b', color: '#ff8c00', label: 'pode demandar instalação de novos terminais' }
  ];
